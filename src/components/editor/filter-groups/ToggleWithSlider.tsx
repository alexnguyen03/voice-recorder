...
export const Toggle: React.FC<ToggleProps> = ({ label, helper, checked, disabled, onChange }) => (
  <label className={`flex items-center gap-2.5 p-2 rounded-md cursor-pointer transition-all
    ${checked
      ? "bg-white/10 dark:bg-white/5 ring-1 ring-inset ring-white/10"
  ...
  >
    ...
    <span className="flex flex-col min-w-0">
      ...
      <span className="text-[9px] text-slate-500 leading-tight mt-0.5">{helper}</span>
    </span>
  </label>
);
...
export const ToggleWithSlider: React.FC<ToggleWithSliderProps> = ({
  ...
  <div className="flex flex-col gap-2">
    ...
    />
    <div className={`pl-3 transition-opacity duration-200 ${toggleChecked ? "opacity-100" : "opacity-35 pointer-events-none"}`}>
      <Slider
