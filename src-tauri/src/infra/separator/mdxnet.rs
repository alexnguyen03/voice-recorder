...
const MDX_MODEL_URL: &str =
    "https://huggingface.co/seanghay/uvr-mdx-net/resolve/main/UVR-MDX-NET-Inst_HQ_3.onnx";
...
impl MdxNetStrategy {
    pub fn load(app: &AppHandle, progress_cb: impl Fn(u64, u64)) -> Result<Self, String> {
        ...
        let path = cache.ensure(MDX_MODEL_URL, MDX_MODEL_FILE, MDX_MIN_BYTES, progress_cb)?;
        Self::from_path(&path)
    }
